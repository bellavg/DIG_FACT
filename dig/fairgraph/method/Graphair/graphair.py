...
from torch_geometric.utils import from_scipy_sparse_matrix, to_dense_adj, to_torch_sparse_tensor, to_edge_index, \
    add_remaining_self_loops, to_undirected
...
class graphair(nn.Module):
    ...
    def __init__(self, aug_model, f_encoder, sens_model, classifier_model, lr=1e-4, weight_decay=1e-5, alpha=10.0,
                 beta=0.1, gamma=0.5, lam=0.5, dataset='POKEC', num_hidden=64, num_proj_hidden=64):
    ...
    def fit_batch(self, epochs, adj, x, sens, idx_sens, warmup=None, adv_epoches=1):
        ...
        for epoch_counter in range(epochs):
            for data in miniBatchLoader:
                ...
                for _ in range(sens_epoches):
                    s_pred, _ = self.sens_model(adj_aug_nograd, x_aug_nograd)
                    senloss = torch.nn.BCEWithLogitsLoss(weight=data.node_norm, reduction='sum')(s_pred[mask].squeeze(),
                                                                                                 data.sens[
                                                                                                     mask].float())
                ...
                s_pred, _ = self.sens_model(adj_aug, x_aug)
                senloss = torch.nn.BCEWithLogitsLoss(weight=data.node_norm, reduction='sum')(s_pred[mask].squeeze(),
                                                                                             data.sens[mask].float())

                ## update aug model
                logits, labels = self.info_nce_loss_2views(torch.cat((h, h_prime), dim=0))
                contrastive_loss = (
                            nn.CrossEntropyLoss(reduction='none')(logits, labels) * data.node_norm.repeat(2)).sum()
